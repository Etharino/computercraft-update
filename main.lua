...
local function threatColor(dist, staleFlag)
...
local function worldToMap(dx, dz, cx, cy, radiusX, radiusY)
...
while true do
    ...
    elseif event == "timer" and a == timer then
        ...
        if not centerPos then
            ...
            put(chars, fgs, bgs, 1, 2, "R" .. RANGE, colors.green)
            ...
            if waypoint and waypoint ~= "SET" then
                ...
                local mx, my = worldToMap(wx, wz, cx, cy, radiusX, radiusY)

                if math.sqrt(wx * wx + wz * wz) <= RANGE then
            ...
            for _, p in ipairs(players) do
                local mx, my = worldToMap(p.dx, p.dz, cx, cy, radiusX, radiusY)
                ...
                local color = threatColor(p.dist, p.stale)
