...
local C = {
    ...
    RV_RATE      = 20,    -- K/s removed while active (tuned for tight meltdown saves)
    RV_DURATION  = 10,
    RV_COOLDOWN  = 90,
...
local rx = {
    ...
    meltdownMult   = 0,
    meltdownSaved  = false,
    scramCooldown  = 0,   -- seconds remaining before SCRAM can be pressed again
...
local turbines = {
    {
        ...
        flowRate=C.FLOW_OPT, flowStep=0, rpmSpeed="M",
...
-- ║  NOTE-BLOCK JINGLES                                  ║
-- ╚══════════════════════════════════════════════════════╝
-- Each jingle is a list of notes: {instrument, pitch(0-24), gap_after}
-- gap_after is in seconds before the next note plays.
-- Pitch 12 = F#4 (default note pitch). CC pitch range is 0-24.
--
-- These are original mood-matched compositions (tense alarm sting,
-- urgent high-temp drone, mechanical startup fanfare, calm ambient
-- loop) — not transcriptions of copyrighted tracks.

local jingleQueue = {}     -- list of pending {instrument,pitch,gap}
local jingleActive = false
local jingleName = nil
local jingleTimer = 0      -- countdown to next note in queue
local jingleLoop = false   -- whether to restart automatically

local JINGLES = {
    -- SCRAM: sharp descending alarm sting, urgent and mechanical
    scram = {
        loop=false,
        notes={
            {"bit",   18, 0.10},
    ...
    -- HIGH TEMP (>2400K): rising tense pulse, repeats while condition holds
    hightemp = {
        loop=true,
        notes={
            {"bass",  6,  0.18},
    ...
    -- IGNITION: mechanical ascending fanfare, plays once on startup
    startup = {
        loop=false,
        notes={
            {"bass",  0,  0.16},
    ...
    -- JUST AFTER STARTUP: calm settling ambient loop, plays once
    online = {
        loop=false,
        notes={
            {"harp",  12, 0.28},
...
local function playJingle(name, force)
    local def = JINGLES[name]
    if not def then return end
    if jingleActive and jingleName==name and not force then return end
    jingleQueue = {}
    for _,n in ipairs(def.notes) do
        table.insert(jingleQueue, {n[1], n[2], n[3]})
    end
    jingleActive = true
    jingleName   = name
    jingleLoop   = def.loop
...
local function stopJingle(name)
    if jingleName==name or name==nil then
        jingleQueue  = {}
        jingleActive = false
        jingleName   = nil
        jingleLoop   = false
...
-- Advances the jingle player by dt seconds; call once per physics tick.
local function updateJingle(dt)
    if not jingleActive or not spk then return end
    jingleTimer = jingleTimer - dt
    if jingleTimer > 0 then return end

    if #jingleQueue == 0 then
        if jingleLoop then
            playJingle(jingleName, true)
        else
            jingleActive = false
            jingleName   = nil
        end
    ...
    local note = table.remove(jingleQueue, 1)
    pcall(function() spk.playNote(note[1], 1.0, note[2]) end)
    jingleTimer = note[3]
end
...
local function doSCRAM(reason)
    ...
    if rx.scramCooldown>0 then return end
    rx.rodPos       = 100
    rx.meltdownMult = 0
    rx.phase        = 5
    rx.scramCooldown= 30
...
local function updatePhysics()
    ...
    -- SCRAM button lockout countdown
    if rx.scramCooldown>0 then
        rx.scramCooldown=math.max(0,rx.scramCooldown-dt)
    end

    -- Advance any playing note-block jingle
    updateJingle(dt)
    ...
    elseif rx.phase==4 then
        -- Heat generation scales with rod withdrawal (0 rods out = no heat,
        -- 100% rods out = max heat far above meltdown if uncooled)
        local heatTarget = lerp(C.T_STALL, C.T_MELTDOWN*1.35, rodEffect)

        -- Cooling systems pull the equilibrium down. Coolant and feedwater
        -- both contribute real, additive cooling power so a stable
        -- midpoint actually exists depending on rod position.
        local coolantPower   = rx.coolantOn   and 650 or 0
        local feedwaterPower = rx.feedwaterOn and 350 or 0
        local totalCooling   = coolantPower + feedwaterPower

        local tgt = heatTarget - totalCooling
        tgt = math.max(C.T_STALL, tgt)

        -- Approach target smoothly. Rate doesn't change near meltdown,
        -- so the reactor genuinely converges to (or overshoots into)
        -- whatever equilibrium the rods/cooling combination implies.
        rx.temperature = lerp(rx.temperature, tgt, dt*0.012)
    ...
    elseif rx.phase==5 then
        if not rx.meltdownActive then
            -- Normal post-scram cooldown (no active meltdown)
            local cr=rx.coolantOn and 0.05 or 0.025
            rx.temperature=lerp(rx.temperature,C.T_STALL,dt*cr)
        end
        -- (If a meltdown IS active, temperature is governed entirely
        --  by the meltdown block above — RVs are the only real lever.)
    ...
    -- (Feedwater cooling is now properly folded into the phase-4
    --  equilibrium calculation above, not applied as a separate hack.)
    ...
    if rx.phase==4 or rx.phase==5 then
        ...
        -- High-temp jingle: plays on loop above 2400K, stops once below
        if rx.temperature>=2400 then
            playJingle("hightemp")
        elseif jingleName=="hightemp" then
            stopJingle("hightemp")
        end
    elseif jingleName=="hightemp" then
        stopJingle("hightemp")
    ...
    for _,t in ipairs(turbines) do
        ...
        if t.broken or not t.online then
            ...
            local sMult=t.rpmSpeed=="S" and 1.0
                        or t.rpmSpeed=="M" and 2.2 or 4.5
            ...
            if t.synced then
                ...
                t.rpm=lerp(t.rpm,rpmTgt,dt*0.04*sMult)
            end
    ...
    if spk and hasAlarms() and alarmFlash and not jingleActive then
...
local function drawTurbinePanel(m,t,px,py,pW,pH)
    ...
    if t.online and not t.synced and not t.broken then
        ...
        local nearSync=(angN<25 or angN>335) and math.abs(t.rpmDelta)<60
...
local function drawECCS()
    ...
    local scramBG, scramFG
    local locked = rx.scramCooldown>0
    if locked then
        scramBG=colors.gray
        scramFG=colors.lightGray
    elseif rx.phase==5 then
        scramBG=alarmFlash and colors.orange or colors.red
        scramFG=colors.white
    ...
    mw(m,sbX+math.floor((sbW-9)/2),row+1,"EMERGENCY",scramFG,scramBG)
    mw(m,sbX+math.floor((sbW-5)/2),row+2,"SCRAM",scramFG,scramBG)
    if locked then
        local lockTxt=string.format("LOCKED %2.0fs",rx.scramCooldown)
        mw(m,sbX+math.floor((sbW-#lockTxt)/2),row+3,lockTxt,colors.black,scramBG)
    elseif rx.phase==5 then
...
local function repairTurbine(t)
    ...
    t.rpm=0; t.scopeAngle=0; t.flowRate=C.FLOW_OPT; t.flowStep=0
...
local function doAction(id)
    ...
    elseif id=="SCRAM" then
        if rx.scramCooldown>0 then
            log(string.format("SCRAM locked – %.0fs remaining",rx.scramCooldown))
        else
